...
// Hilfsfunktion für lineare Interpolation zwischen zwei Punkten
function interpolatePoints(p1: [number, number], p2: [number, number], steps: number): [number, number][] {
...
export function createSmoothAltitudeSegments(routePoints: FlightTrackPoint[]): Array<{
  ...
  const curveResolution = 256; // Für Kurven
  const minLinearSteps = 20; // Mindestens 20 Zwischenpunkte für lange Strecken
  ...
  controlPointSets.forEach((controlPoints, segmentIndex) => {
    ...
    // Abstand zwischen Punkten berechnen (einfache euklidische Distanz)
    const dist = Math.sqrt(
      Math.pow(controlPoints.p3[0] - controlPoints.p0[0], 2) +
      Math.pow(controlPoints.p3[1] - controlPoints.p0[1], 2)
    );

    // Wenn die Strecke fast gerade ist oder sehr lang, lineare Interpolation
    if (dist > 0.2) { // Schwellenwert ggf. anpassen
      const linearPoints = [controlPoints.p0, ...interpolatePoints(controlPoints.p0, controlPoints.p3, minLinearSteps), controlPoints.p3];
      for (let i = 0; i < linearPoints.length - 1; i++) {
        const t1 = i / (linearPoints.length - 1);
        const t2 = (i + 1) / (linearPoints.length - 1);
        const avgT = (t1 + t2) / 2;
        const segmentColor = interpolateColor(currentColor, nextColor, avgT);
        segments.push({
          coordinates: [linearPoints[i], linearPoints[i + 1]],
          color: segmentColor,
          opacity: 0.9
        });
      }
    } else {
      // Bézier-Kurve für Kurven
