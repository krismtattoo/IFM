...
const LeafletAircraftMarker: React.FC<LeafletAircraftMarkerProps> = ({ 
  ...
  const createAircraftIcon = useCallback((flight: Flight, isSelected: boolean = false): L.DivIcon => {
    ...
    // a different color for ground in dark mode, and darker grey/blue-grey in light mode.
    const lightModeGroundColor = '#9ca3af'; // Darker grey for light mode (ground)
    const lightModeAirborneColor = '#475569'; // Dark blue-grey for light mode (airborne)
    const darkModeAirborneColor = '#d1d5db'; // Light grey for dark mode (airborne)
    const darkModeGroundColor = '#94a3b8'; // Slightly darker grey/blue for dark mode (ground)
    const selectedColor = '#5DADEC'; // Light blue matching the image (selected)
    
    let fillColor;
    if (isSelected) {
      fillColor = selectedColor;
    } else if (isDarkMode) {
      fillColor = onGround ? darkModeGroundColor : darkModeAirborneColor; // Use specific colors for dark mode based on ground status
    } else {
      fillColor = onGround ? lightModeGroundColor : lightModeAirborneColor; // Use darker colors in light mode
    }
...
export default React.memo(LeafletAircraftMarker);
