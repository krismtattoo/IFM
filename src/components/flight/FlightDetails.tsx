...
const FlightDetails: React.FC<FlightDetailsProps> = ({ flight, serverID, onClose }) => {
  ...
  const renderRouteSection = () => {
    ...
    return (
      <div className="space-y-6">
            ...
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                {flightPlanData.departure && (
                  <div className="text-center">
                    <div className="text-sm text-blue-200 mb-1">Abflug</div>
                ...
                {flightPlanData.destination && (
                  <div className="text-center">
                    <div className="text-sm text-blue-200 mb-1">Ankunft</div>
        ...
        {/* Waypoints List with Custom ScrollArea */}
        <Card className="bg-slate-800 border-slate-700">
          <CardHeader>
  ...
  const renderPerformanceSection = () => {
    // Nutze die echte Telemetrie-Historie, falls vorhanden
    const hasTrack = Array.isArray(flight.track) && flight.track.length > 0;
    const performanceData = hasTrack
      ? flight.track.map((point, idx, arr) => {
          // Berechne verticalSpeed aus Altitude-Differenz pro Minute
          let verticalSpeed = 0;
          if (idx > 0) {
      ...
      <Card className="bg-slate-800 border-slate-700">
        ...
        <CardContent className="space-y-4">
          <div>
            ...
            <div className="text-sm text-gray-400 select-none cursor-pointer flex items-center gap-2">
              <span onClick={() => setShowFlightId(v => !v)} className="underline hover:text-blue-400 transition-colors">
                {showFlightId ? 'Hide Flight ID' : 'Show Flight ID'}
              </span>
              {showFlightId && (
                <span className="font-mono text-xs bg-slate-700 px-2 py-1 rounded text-gray-300 border border-slate-600">
                  {flight.flightId}
                </span>
              )}
            </div>
      ...
      <>
        ...
        {isMinimized && (
          <button
            ...
            className="fixed bottom-4 left-4 z-50 bg-slate-800 text-white p-3 rounded-full shadow-lg border border-slate-700 hover:bg-slate-700 transition-colors"
          >
        ...
        {/* Bottom Sheet */}
        <div 
          className={`fixed bottom-0 left-0 right-0 z-40 bg-slate-900 border-t border-slate-700 rounded-t-2xl shadow-2xl transition-transform duration-300 ${
