...
const VerticalSpeedChart: React.FC<VerticalSpeedChartProps> = ({ data }) => {
    ...
    if (active && payload && payload.length) {
      ...
      return (
        <div className="bg-gray-900/95 backdrop-blur-sm border border-gray-700 rounded-xl p-3 shadow-2xl">
          <p className="text-white font-semibold mb-2 text-sm">{`Time: ${label}`}</p>
          <div className="flex items-center gap-2">
            <div className={`w-3 h-3 rounded-full ${
              value > 0 ? 'bg-green-500' : value < 0 ? 'bg-red-500' : 'bg-gray-500'
            }`} />
            <span className="text-gray-300 text-xs">
              Vertical Speed: {value > 0 ? '+' : ''}{value.toFixed(0)}fpm
            </span>
          </div>
        </div>
      );
  ...
  return (
    <Card className="border-slate-700 shadow-lg bg-transparent">
      ...
      <CardContent className="p-4">
        <div className="h-48 w-full">
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={data} margin={{ top: 10, right: 20, left: 20, bottom: 5 }} style={{ background: 'transparent' }}>
